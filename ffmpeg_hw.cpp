...
{
    HWDevice* FFmpeg::hw_device_get_by_type(enum AVHWDeviceType type)
    {
        HWDevice* found = NULL;
        int i;
    ...
    HWDevice* FFmpeg::hw_device_get_by_name(const char* name)
    {
        int i;
        for (i = 0; i < nb_hw_devices; i++) {
    ...
    HWDevice* FFmpeg::hw_device_add(void)
    {
        int err;
    ...
    char* FFmpeg::hw_device_default_name(enum AVHWDeviceType type)
    {
    ...
    int FFmpeg::hw_device_init_from_string(const char* arg, HWDevice** dev_out)
    {
    ...
    int FFmpeg::hw_device_init_from_type(enum AVHWDeviceType type,
        const char* device,
        HWDevice** dev_out)
    {
        AVBufferRef* device_ref = NULL;
    ...
    void FFmpeg::hw_device_free_all(void)
    {
        int i;
        for (i = 0; i < nb_hw_devices; i++) {
    ...
    HWDevice* FFmpeg::hw_device_match_by_codec(const AVCodec* codec)
    {
    ...
    int FFmpeg::hw_device_setup_for_decode(InputStream* ist)
    {
        const AVCodecHWConfig* config;
        enum AVHWDeviceType type;
        HWDevice* dev = NULL;
    ...
    int FFmpeg::hw_device_setup_for_encode(OutputStream* ost)
    {
        const AVCodecHWConfig* config;
        HWDevice* dev = NULL;
    ...
    int FFmpeg::hwaccel_retrieve_data(AVCodecContext* avctx, AVFrame* input)
    {
    ...
    int FFmpeg::hwaccel_decode_init(AVCodecContext* avctx)
    {
        InputStream* ist = static_cast<InputStream*>(avctx->opaque);
    ...
    int FFmpeg::hw_device_setup_for_filter(FilterGraph* fg)
    {
        HWDevice* dev;
        int i;
